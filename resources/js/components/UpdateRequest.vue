<script setup>
	...
	const searchBtn = () => {
		...
		axios.get(props.api + "/inventory?search=" + search.value)
		.then((response) => {
	...
	const selectItem = (item) => {
		...
		if (!isInArray) {
			const temp = {
				...
				name: item.name,
				quantity: 1,
				max: item.max ? item.max : item.quantity,
				brand: item.brand,
	...
	const removeItem = (index) => {
		const id = form.value[index].item_id;
		deleteForm.value.push(id);
